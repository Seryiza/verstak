...
- `--allow-host-programs NAMES`: comma-separated host PATH program names to expose in the VM through host-side policy-enforced proxy stubs, for example `git,gh`.
- `--host-programs-policy PATH`: structured JSON policy file with `allow` and `forbid` host-program rules.
...
## Host program allowlist

Host-program delegation is denied by default. When configured, Verstak exposes
selected host tools inside the VM as normal commands in `${HOME}/.local/bin`,
which is first in the generated PATH. For example:

```sh
verstak --allow-host-programs git --one-shot git status
```

The guest command is only a client stub. Enforcement happens on the host side
through a QEMU `guestfwd` command proxy, so modifying guest wrappers or using
sudo in the VM does not grant additional host programs. The proxy validates the
...
proxy streams stdin/stdout/stderr and returns the host command's exit status.
Audit events are appended under the Verstak state directory at
`host-programs/audit.jsonl`; events include program, argv, cwd mapping,
decision, reason, and exit status, but not stdin/stdout/stderr transcripts.
...
- `VERSTAK_HOST_PROGRAM_PROXY_ADDR`: internal guest client override for the host-program proxy address. Normally generated by Verstak stubs; useful only for debugging custom stubs.
