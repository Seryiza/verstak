...
let
  ...
  claudeTools = import ../tools/claude.nix {
    inherit
      config
      lib
      llmAgents
...
in
{
  config = lib.mkIf cfg.claude.enable {
    environment = {
      systemPackages = claudeTools.packages;
      ...
      etc = {
        ...
        "claude/claude.json".text = builtins.toJSON { theme = "light-daltonized"; };
    ...
    systemd.services.verstak-claude-auth = {
      ...
      serviceConfig = {
        Type = "oneshot";
        ExecStart = "${claudeTools.seedClaudeAuth}/bin/verstak-seed-claude-auth";
      };
    };
